...
class Curl {
    ...
    private static function _Process ($url, $options=array()) {
        ...
        if(!isset($options['ua'])){ $options['ua'] = 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0'; }
        // 设置选项默认值
        ...
        if ( $result===FALSE || $httpInfo['http_code'] != 200 ) {
            $msg = curl_error($curl).' ('.curl_errno($curl).')';
